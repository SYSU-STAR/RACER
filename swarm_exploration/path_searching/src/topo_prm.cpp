...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
    vector<Eigen::Vector3d> start_pts, vector<Eigen::Vector3d> end_pts, list<GraphNode::Ptr>& graph,
  ...
  if (graph_.size() > 2) {
    for (list<GraphNode::Ptr>::iterator iter1 = graph_.begin();
      ...
      if ((*iter1)->neighbors_.size() <= 1) {
        ...
        for (list<GraphNode::Ptr>::iterator iter2 = graph_.begin(); iter2 != graph_.end();
             ++iter2) {
...
vector<vector<Eigen::Vector3d>> TopologyPRM::pruneEquivalent(
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(
    const vector<Eigen::Vector3d>& path, int pt_num) {
...
void TopologyPRM::pathToGuidePts(
    vector<Eigen::Vector3d>& path, const int& pt_num, vector<Eigen::Vector3d>& pts) {
...
vector<vector<Eigen::Vector3d>> TopologyPRM::discretizePaths(
    vector<vector<Eigen::Vector3d>>& path) {
