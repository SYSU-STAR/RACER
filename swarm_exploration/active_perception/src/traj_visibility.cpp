...
Eigen::Vector3d VisibilityUtil::getMinDistVoxel(const Eigen::Vector3d& q1,
    const Eigen::Vector3d& q2, const Eigen::Vector3d& offset, const double& res) {
...
Eigen::Vector3d VisibilityUtil::getMinDistVoxelOnLine(const Eigen::Vector3d& q1,
    const Eigen::Vector3d& q2, const Eigen::Vector3d& offset, const double& res, int& state,
...
bool VisibilityUtil::lineVisib(
    const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, Eigen::Vector3d& pc) {
...
void VisibilityUtil::findVisibPairs(
    const vector<Eigen::Vector3d>& pts, vector<VisiblePair>& pairs) {
  int know_num = 0;
  pairs.clear();
  int cur_j, prev_j = -1;
...
bool VisibilityUtil::findUnknownPoint(
    NonUniformBspline& traj, Eigen::Vector3d& point, double& time) {
...
bool VisibilityUtil::findDirAndIdx(NonUniformBspline& traj, const double& unknown_t,
    const double& crit_t, Eigen::Vector3d& dir, int& idx, Eigen::Vector3d& min_pt) {
...
void VisibilityUtil::calcViewConstraint(NonUniformBspline& traj, ViewConstraint& cons) {
  ...
  cons.idx_ = idx;
  cons.pcons_ = min_pt;
  // ROS_WARN_STREAM("[view] unknown pt: " << cons.pt_.transpose() << ", dir: " <<
  // cons.dir_.transpose()
  //                                       << ", idx: " << cons.idx_);
}
...
vector<Eigen::Vector3d> VisibilityUtil::precomputeForVisibility(
    const vector<Eigen::Vector3d>& ctrl_pts, bool debug) {
