...
int main(int argc, char** argv) {
  ...
  while (ros::ok()) {
    ...
    bool success =
        planner_manager->benckmarkReplan(start_pt, start_vel, start_acc, end_pt, end_vel);
    ...
    if (!success) {
      ...
      visualization->drawBspline(planner_manager->position_traj_, 0.1,
          Eigen::Vector4d(1.0, 0.0, 0.0, 1), false, 0.1, Eigen::Vector4d(1.0, 1.0, 1.0, 1), 99);
